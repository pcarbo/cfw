# This is Peter's script for estimating LD decay in the Diversity
# Outbred (DO) mice.
#
...
geno.file <- "/tmp/pcarbo/data/DO/gigamuga_do"
...
map <- read.table("/tmp/pcarbo/data/DO/gigamuga_do.bim",
...
system(sprintf(paste("~/bin/plink2 --bfile %s --freq --chr %d",
                     "--out /tmp/pcarbo/plink/do"),
               geno.file,which.chr))
out <- read.table("/tmp/pcarbo/plink/do.frq",header = TRUE,
                  stringsAsFactors = FALSE)
